...
| 내가 적은 것 | 정확한 내용 |
|---|---|
| `docker build -t` |  `-t`는 이름(태그) 붙이기 |
| `docker ps -a` (all?) |  `-a` = `--all` 전체 목록 |
| `run -p 80:8000 -d` (background?) | `-d` = detached = 백그라운드 실행 |
| `docker -it docker -db -1 bash` | 정확한 명령어는 `docker exec -it <컨테이너ID> bash` |
| 나오는 방법 `Ctrl+D` | `exit` 명령어도 동일 |
| 다수의 컨테이너를 손쉽게 → docker compose |  |
