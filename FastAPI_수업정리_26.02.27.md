...
| **비동기 (async)**  | 대기 시간 동안 다른 작업을 처리 |
| **async def**     | 비동기 함수(코루틴 함수) 정의 |
| **await**         | 대기가 발생하는 순간 실행권을 양보하는 키워드 |
| **gather()**      | 여러 코루틴을 동시에 실행 |
| **blocking**      | 비동기 환경에서 실행권을 양보하지 않는 나쁜 코드 |
| **BackgroundTasks** | 응답을 먼저 보내고 무거운 작업을 백그라운드에서 실행 |
